...
# Script to update nginx configuration in kason-ui container
...
if [ -d "$CURRENT_DIR/microservice-kason/kason-deploy/docker" ]; then
  chmod 777 "$CURRENT_DIR/microservice-kason/kason-deploy/docker"/*.sh 2>/dev/null || true
fi
if [ -d "$CURRENT_DIR/microservice-kason/kason-deploy/kason-ui" ]; then
...
KASON_CONTAINER_NAME=${KASON_CONTAINER_NAME:-kason-ui}

if ! docker ps --format '{{.Names}}' | grep -q "^${KASON_CONTAINER_NAME}$"; then
  echo "✗ Container '${KASON_CONTAINER_NAME}' not found or not running."
  echo "  Tip: docker ps --format 'table {{.Names}}\t{{.Status}}'"
  exit 1
fi

echo "Updating nginx configuration in container: ${KASON_CONTAINER_NAME}"
...
docker exec "${KASON_CONTAINER_NAME}" sh -c 'cp -f /etc/nginx/nginx.conf /etc/nginx/nginx.conf.backup 2>/dev/null || true'

# Write the new nginx configuration using a quoted heredoc to preserve $nginx_vars
cat <<'EOF' | docker exec -i "${KASON_CONTAINER_NAME}" tee /etc/nginx/nginx.conf >/dev/null
user nginx;
...
docker exec "${KASON_CONTAINER_NAME}" sh -c \
  "sed -i 's|@BACKEND@|${BACKEND_URL//|/\\|}|g' /etc/nginx/nginx.conf"

# Validate and reload
set +e
docker exec "${KASON_CONTAINER_NAME}" nginx -t
NGINX_TEST_RC=$?
set -e
...
if [ $NGINX_TEST_RC -eq 0 ]; then
  ...
  docker exec "${KASON_CONTAINER_NAME}" nginx -s reload || true
  docker container restart "${KASON_CONTAINER_NAME}" >/dev/null
  ...
  docker exec "${KASON_CONTAINER_NAME}" sh -c 'cp -f /etc/nginx/nginx.conf.backup /etc/nginx/nginx.conf 2>/dev/null || true'
  docker exec "${KASON_CONTAINER_NAME}" nginx -s reload || true
...
To check nginx status and logs:
  docker exec ${KASON_CONTAINER_NAME} nginx -t                    # Test configuration
  docker exec ${KASON_CONTAINER_NAME} nginx -s reload             # Reload configuration
  docker logs ${KASON_CONTAINER_NAME}                             # View container logs
  docker exec -it ${KASON_CONTAINER_NAME} tail -f /var/log/nginx/access.log  # Access logs
  docker exec -it ${KASON_CONTAINER_NAME} tail -f /var/log/nginx/error.log   # Error logs
