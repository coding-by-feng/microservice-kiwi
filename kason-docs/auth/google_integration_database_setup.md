...
You need to run two SQL scripts in the correct order to set up Google OAuth2 integration with your Kason system.
...
mysql -u your_username -p kason_db < oauth2-tables.sql
```
...
mysql -u your_username -p kason_db < database-migration.sql
```
...
-- Should show:
-- kason-client (traditional auth)
-- google-sso-client (Google OAuth)
-- kason-mobile-client (mobile app)
```
...
### 1. Traditional Kason Client
```
Client ID: kason-client
...
spring:
  datasource:
    url: jdbc:mysql://localhost:3306/kason_db
...
export KASON_ENC_PASSWORD="your_encryption_password"
```
...
curl -X POST http://localhost:3001/oauth/token \
  ...
  -d "grant_type=password&username=test&password=123456&client_id=kason-client&client_secret=secret"
...
mysqldump -u username -p kason_db > kason_db_backup_$(date +%Y%m%d_%H%M%S).sql

# Create table-specific backups
mysqldump -u username -p kason_db sys_user > sys_user_backup_$(date +%Y%m%d_%H%M%S).sql
```
