...
import me.fengorz.kason.crawler.service.FetchService;
import me.fengorz.kason.crawler.service.JsoupService;
import me.fengorz.kason.crawler.util.CrawlerUtils;
...
import me.fengorz.kason.word.api.dto.queue.*;
...
import me.fengorz.kason.word.api.exception.PhraseRemoveException;
import me.fengorz.kason.word.api.exception.WordRemoveException;
...
public class FetchServiceImpl implements FetchService {
    ...
    public void handle(FetchWordMqDTO messageDTO) {
        ...
        try {
            ...
            if (storeResult.isFail()) {
                ...
                if (KasonStringUtils.isNotEquals(inputWord, fetchWord)) {
                    queryApi.insertVariant(inputWord, fetchWord);
                    String insertVariantResult = KasonStringUtils
                ...
                String fetchResult = KasonStringUtils.format("word({}) fetch store success! spent {}s",
    ...
    public void retrievePhrase(FetchPhraseRunUpMqDTO dto) {
        ...
        try {
            ...
            if (KasonCollectionUtils.isNotEmpty(resultDTO.getPhrases())) {
    ...
    private void handleException(FetchQueueDO queue, int status, String message) {
        ...
        queue.setIsLock(GlobalConstants.FLAG_NO);
        if (KasonStringUtils.isNotBlank(message)) {
            log.error(message);
        }
    }
}
