...
package me.fengorz.kason.crawler.component.producer.phrase.async;
...
public class FetchRunUpProducer extends AbstractProducer implements MqProducer {
    ...
    @Override
    public void produce() {
        log.info("FetchRunUpProducer produce method is starting");
        super.produce(ApiCrawlerConstants.STATUS_ALL_SUCCESS);
        log.info("FetchRunUpProducer produce method has ended");
    }
    ...
    protected void execute(FetchQueueDO queue) {
        mqSender.fetchPhraseRunUp(new FetchPhraseRunUpMqDTO().setQueueId(queue.getQueueId())
            .setWord(KasonStringUtils.isBlank(queue.getDerivation()) ? queue.getWordName() : queue.getDerivation())
