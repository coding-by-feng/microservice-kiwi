...
run_easy_check() {
  ...
  if sudo_nopass_ready; then
    ...
    log "WARN: sudo without password is not configured. Starting easy-check in tmux window 'kiwi'."
    ensure_tmux_window kiwi || return 1
    tmux send-keys -t "${SESSION_NAME}:kiwi" "sudo '$easy_check'" C-m
    ...
    tmux send-keys -t "${SESSION_NAME}:kiwi" "23" C-m "y" C-m C-m "0" C-m
    log "If password was required by sudo, attach to tmux and complete manually: tmux attach -t ${SESSION_NAME} (window: kiwi)"
  fi
}

run_easy_deploy() {
  local easy_deploy
  if ! easy_deploy=$(resolve_path "easy-deploy" "$HOME/easy-deploy" "$SCRIPT_DIR/docker/deployKiwi.sh"); then
    log "ERROR: easy-deploy not found. Expected at: $HOME/easy-deploy or $SCRIPT_DIR/docker/deployKiwi.sh"
    return 1
  fi
  ...
  if sudo_nopass_ready; then
    ...
    ensure_tmux_window kiwi || return 1
    tmux send-keys -t "${SESSION_NAME}:kiwi" "sudo -E '$easy_deploy' -mode=ouej 2>&1 | tee -a '$LOG_FILE'" C-m
    log "Started easy-deploy in tmux window 'kiwi'. If prompted for password, attach: tmux attach -t ${SESSION_NAME}"
  fi
}
