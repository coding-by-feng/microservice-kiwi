...
package me.fengorz.kason.crawler.service.impl;
...
import me.fengorz.kason.crawler.common.enumeration.JsoupRootTypeEnum;
import me.fengorz.kason.crawler.constant.CrawlerSourceEnum;
import me.fengorz.kason.crawler.service.JsoupService;
import me.fengorz.kason.crawler.util.CrawlerAssertUtils;
import me.fengorz.kason.word.api.common.ApiCrawlerConstants;
import me.fengorz.kason.word.api.dto.queue.FetchPhraseMqDTO;
import me.fengorz.kason.word.api.dto.queue.FetchPhraseRunUpMqDTO;
import me.fengorz.kason.word.api.dto.queue.FetchWordMqDTO;
...
public class JsoupServiceImpl implements JsoupService {
    ...
    private static final String JSOUP_FETCH_IN_CHINESE_EXCEPTION =
            "Occurred exception in the jsoup fetching in Chinese, the word is {}, Kason is trying to fetch in English.";
    ...
    public FetchPhraseResultDTO fetchPhraseInfo(FetchPhraseMqDTO dto)
        ...
        if (KasonCollectionUtils.isNotEmpty(paraphraseElements)) {
            ...
            for (Element element : elements) {
                Optional.ofNullable(element.getElementsByClass("headword hdb tw-bw dhw dpos-h_hw "))
                        .ifPresent(singlePhraseElement -> {
                            ...
                            if (KasonStringUtils.equals(word, finalPhrase)) {
                                return;
                            }
        ...
        if (phraseElements != null && phraseElements.size() > 0) {
            for (Element phraseElement : phraseElements) {
                ...
                Optional.ofNullable(href.get(0).attr("title")).ifPresent(phrase -> {
                    // 如果是单词本身跳过
                    if (KasonStringUtils.equals(word, phrase)) {
                        return;
                    }
                    // 后面附带"XXX"多余的结尾要去掉
                    ...
                    phrase = phrase.trim();
                    if (KasonStringUtils.equals(phrase, word)) {
                        return;
                    }
                    phrases.add(phrase);
                });
    ...
    private FetchWordResultDTO fetch(String url, String word)
        ...
        KasonAssertUtils.assertNotEmpty(jsoupWord, FETCH_MAIN_WORD_NAME_EXCEPTION, word);
            ...
            if (phrasesIsEmpty) {
                ...
                for (Element phraseBlock : phrases) {
                    ...
                    for (int i = 0; i < subPhrases.size(); i++) {
                        ...
                        if (KasonStringUtils.equals(subPhrasesParaphrase, paraphraseEnglishText)) {
