...
public class YtbSubtitleStreamingService {
    ...
    @KasonCacheKeyPrefix(AiConstants.CACHE_KEY_PREFIX_GROK.SUBTITLE_SCROLLING)
    @Cacheable(cacheNames = AiConstants.CACHE_NAMES, keyGenerator = CacheConstants.CACHE_KEY_GENERATOR_BEAN,
            unless = "#result == null")
    public YtbSubtitlesResult getScrollingSubtitlesInternal(@KasonCacheKey(1) String videoId, String decodedUrl) {
    ...
    @Cacheable(cacheNames = AiConstants.CACHE_NAMES, keyGenerator = CacheConstants.CACHE_KEY_GENERATOR_BEAN,
            unless = "#result == null")
    public String getVideoTitleInternal(@KasonCacheKey(1) String videoId, String decodedUrl) {
        log.info("Fetching video title for videoId: {}", videoId);
        return youTuBeHelper.getVideoTitle(decodedUrl);
    }

    @KasonCacheKeyPrefix(AiConstants.CACHE_KEY_PREFIX_GROK.SUBTITLE_STREAMING)
    @Cacheable(cacheNames = AiConstants.CACHE_NAMES, keyGenerator = CacheConstants.CACHE_KEY_GENERATOR_BEAN,
            unless = "#result == null")
    public String getCachedStreamingSubtitleTranslationById(@KasonCacheKey(1) String videoId,
                                                            @KasonCacheKey(2) String language) {
    ...
    @CachePut(cacheNames = AiConstants.CACHE_NAMES, keyGenerator = CacheConstants.CACHE_KEY_GENERATOR_BEAN,
            unless = "#result == null")
    public String cacheStreamingSubtitleTranslation(@KasonCacheKey(1) String videoId,
                                                    @KasonCacheKey(2) String language,
    ...
    public void cleanScrollingSubtitlesCacheById(@KasonCacheKey(1) String videoId) {
        log.info("Cleaning scrolling subtitles cache for videoId: {}", videoId);
    }

    @KasonCacheKeyPrefix(AiConstants.CACHE_KEY_PREFIX_GROK.VIDEO_TITLE)
    @CacheEvict(cacheNames = AiConstants.CACHE_NAMES, keyGenerator = CacheConstants.CACHE_KEY_GENERATOR_BEAN)
    public void cleanVideoTitleCacheById(@KasonCacheKey(1) String videoId) {
        log.info("Cleaning video title cache for videoId: {}", videoId);
    }

    @KasonCacheKeyPrefix(AiConstants.CACHE_KEY_PREFIX_GROK.SUBTITLE_STREAMING)
    @CacheEvict(cacheNames = AiConstants.CACHE_NAMES, keyGenerator = CacheConstants.CACHE_KEY_GENERATOR_BEAN)
    public void cleanStreamingSubtitleTranslationCacheById(@KasonCacheKey(1) String videoId,
                                                           @KasonCacheKey(2) String language) {
