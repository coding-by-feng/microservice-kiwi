...
public class JsoupServiceImpl implements IJsoupService {
        ...
        Optional.ofNullable(doc.getElementsByClass("hax lp-10 lb lb-cm lbt0 dbrowse")).ifPresent(idiomElements -> {
            for (Element element : idiomElements) {
                Optional.ofNullable(element.getElementsByClass("lmb-12")).ifPresent(elements -> {
                    putPhrase(word, phrases, elements, WordCrawlerConstants.PHRASE_FETCH_VERBOSE_IDIOM, WordCrawlerConstants.PHRASE_FETCH_VERBOSE_IDIOM_LENGTH);
                });
            }
        });
        /*词组抓取 end*/
        /*习语抓取 begin*/
        Optional.ofNullable(doc.getElementsByClass("xref idioms hax dxref-w lmt-25 lmb-25")).ifPresent(idiomElements -> {
            for (Element element : idiomElements) {
                Optional.ofNullable(element.getElementsByClass("item lc lc1 lpb-10 lpr-10")).ifPresent(elements -> {
                    putPhrase(word, phrases, elements, WordCrawlerConstants.PHRASE_FETCH_VERBOSE_MEAN, WordCrawlerConstants.PHRASE_FETCH_VERBOSE_MEAN_LENGTH);
                });
            }
        });
        /*习语抓取 end*/
        /*短语动词抓取 begin*/
        Optional.ofNullable(doc.getElementsByClass("xref phrasal_verbs hax dxref-w lmt-25 lmb-25")).ifPresent(idiomElements -> {
            for (Element element : idiomElements) {
                Optional.ofNullable(element.getElementsByClass("item lc lc1 lc-xs6-12 lpb-10 lpr-10")).ifPresent(elements -> {
                    putPhrase(word, relatedWords, elements, WordCrawlerConstants.PHRASE_FETCH_VERBOSE_MEAN, WordCrawlerConstants.PHRASE_FETCH_VERBOSE_MEAN_LENGTH);
                });
            }
        });
        /*短语动词抓取 end*/
    ...
    public FetchPhraseResultDTO fetchPhraseInfo(FetchPhraseMqDTO dto) throws JsoupFetchConnectException, JsoupFetchResultException {
        ...
        return result.setFetchParaphraseDTOList(paraphraseDTOs).setPhrase(phrase).setQueueId(dto.getQueueId()).setRelatedWords(relatedWords).setDerivation(dto.getDerivation());
    }

    private void putPhrase(String word, Set<String> phrases, Elements phraseElements, String verbose, int verboseLength) {
        ...
        for (Element block : root) {
            ...
            try {
                ...
                Optional.ofNullable(subFetchPronunciation(word, block, KEY_UK_PRONUNCIATIOIN,
                        WordCrawlerConstants.PRONUNCIATION_TYPE_UK, isAlreadyFetchPronunciation)).ifPresent(pronunciationDTOList::add);
                // fetch US pronunciation
                Optional.ofNullable(subFetchPronunciation(word, block, KEY_US_PRONUNCIATIOIN,
                        WordCrawlerConstants.PRONUNCIATION_TYPE_US, isAlreadyFetchPronunciation)).ifPresent(pronunciationDTOList::add);
    ...
    private FetchParaphraseExampleDTO subFetchExamples(List<FetchParaphraseExampleDTO> exampleDTOList, Element sentence) {
        ...
        Optional.ofNullable(sentence.getElementsByClass(KEY_SENTENCE)).ifPresent(
                (Elements elements) -> exampleDTO.setExampleSentence(elements.text()));
    ...
    private FetchWordPronunciationDTO subFetchPronunciation(String word, Element block, String pronunciationKey,
        ...
        return pronunciationDTO.setSoundmarkType(pronunciationType)
