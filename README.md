...
docker run -itd --name kiwi-mysql -p 3306:3306 -v /root/docker/mysql:/mysql_tmp -e MYSQL_ROOT_PASSWORD=changeme --net=host mysql:5.7.34
sudo docker exec -it kiwi-mysql bash
mysql -h localhost -u root -p
create database kiwi_db;
exit
# 迁移Mysql的kiwi_db表
mysqldump --host=cdb-0bhxucw9.gz.tencentcdb.com --port=10069 -uroot -pchangeme -C --databases kiwi_db |mysql --host=localhost -uroot -pchangeme kiwi_db
mysql -h localhost -u root -p
use kiwi_db
...
docker run -itd --name kiwi-redis -p 6379:6379 redis --requirepass "changeme"
# 测试
...
```
# delete old
curl -u changeme:changeme -X POST "http://localhost:9200/_aliases" -H "Content-Type: application/json" -d '{
  "actions": [
    {
      "remove": {
        "index": ".security-7",
