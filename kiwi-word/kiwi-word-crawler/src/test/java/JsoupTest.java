  ...
  public static void main(String[] args) throws IOException {
    ...
    root.forEach(
        block -> {
          ...
          Optional.ofNullable(block.getElementsByClass("pos-header dpos-h"))
              ...
              paraphrases -> {
                ...
                singleParaphrase.forEach(
                    paraphrase -> {
                      ...
                      Assert.notNull(
                          paraphraseEnglish, "The paraphraseEnglish of {} is not found!", word);
                      fetchParaphraseDTO.setParaphraseEnglish(paraphraseEnglish.text());

                      // fetch chinese meaning
                      Elements meaningChinese =
                          paraphrase.getElementsByClass("trans dtrans dtrans-se ");
                      Assert.notNull(
                          meaningChinese, "The meaningChinese of {} is not found!", word);
                      ...
                      if (!exampleSentences.isEmpty()) {
                        List<FetchParaphraseExampleDTO> fetchParaphraseExampleDTOList =
                            new ArrayList<>();
                        exampleSentences.forEach(
                            sentence -> {
                              FetchParaphraseExampleDTO fetchParaphraseExampleDTO =
                                  new FetchParaphraseExampleDTO();
                              Optional.ofNullable(sentence.getElementsByClass("eg deg"))
                                  .ifPresent(
                                      elements -> {
                                        fetchParaphraseExampleDTO.setExampleSentence(
                                            elements.text());
                                      });
                              Optional.ofNullable(
                                      sentence.getElementsByClass("trans dtrans dtrans-se hdb"))
                                  .ifPresent(
                                      elements -> {
                                        fetchParaphraseExampleDTO.setExampleTranslate(
                                            elements.text());
                                      });
