...
### 1. Authentication Service (kason-auth)

#### GoogleOAuth2Controller
- **Location**: `kason-auth/src/main/java/me/fengorz/kason/auth/controller/GoogleOAuth2Controller.java`
...
#### GoogleOAuth2Service
- **Location**: `kason-auth/src/main/java/me/fengorz/kason/auth/service/GoogleOAuth2Service.java`
...
- **Location**: `kason-bdf/src/main/java/me/fengorz/kason/bdf/security/google/GoogleTokenCacheService.java`
...
### 2. User Management Service (kason-upms)

#### GoogleUserService
- **Location**: `kason-auth/src/main/java/me/fengorz/kason/auth/service/GoogleUserService.java`
...
- **Location**: `kason-upms/kason-upms-biz/src/main/java/me/fengorz/kason/upms/controller/GoogleUserController.java`
...
sequenceDiagram
    ...
    participant KasonAuth
    participant Google
    participant Redis
    participant Database

    User->>Frontend: Click "Login with Google"
    Frontend->>KasonAuth: GET /oauth/google/authorize
    KasonAuth->>Frontend: Return authorization URL
    Frontend->>Google: Redirect to Google OAuth2
    Google->>User: Display consent screen
    User->>Google: Grant permissions
    Google->>KasonAuth: Callback with authorization code
    KasonAuth->>Google: Exchange code for tokens
    Google->>KasonAuth: Return access/refresh tokens
    KasonAuth->>Google: Get user information
    Google->>KasonAuth: Return user profile
    KasonAuth->>Database: Create/update user
    KasonAuth->>KasonAuth: Generate system OAuth2 token
    KasonAuth->>Redis: Cache Google token mapping
    KasonAuth->>Frontend: Redirect with system token
```
...
sequenceDiagram
    ...
    Client->>KasonAuth: POST /oauth/google/login {accessToken}
    KasonAuth->>Google: Validate token & get user info
    Google->>KasonAuth: Return user profile
    KasonAuth->>Database: Create/update user
    KasonAuth->>KasonAuth: Generate system OAuth2 token
    KasonAuth->>Redis: Cache token mapping
    KasonAuth->>Client: Return system token & user info
```
...
```
Key Patterns:
- kason:google:token:{systemToken} → GoogleTokenCacheInfo (JSON)
- kason:google:refresh:{googleRefreshToken} → systemToken
- kason:google:mapping:{systemToken} → GoogleTokenCacheInfo (JSON)
- kason:oauth:access_token:{systemToken} → OAuth2AccessToken
```
...
google:
  oauth2:
    ...
    check-token-endpoint-url: http://kason-auth:3001/oauth/check_token
```
