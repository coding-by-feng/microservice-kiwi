...
import me.fengorz.kason.word.api.dto.queue.result.FetchWordReplaceDTO;
import me.fengorz.kason.word.api.entity.*;
...
import me.fengorz.kason.word.api.vo.detail.PronunciationVO;
import me.fengorz.kason.word.api.vo.detail.WordQueryVO;
import me.fengorz.kason.word.biz.service.base.*;
import me.fengorz.kason.word.biz.service.operate.OperateService;
...
@KasonCacheKeyPrefix(WordConstants.CACHE_KEY_PREFIX_OPERATE.CLASS)
public class OperateServiceImpl implements OperateService {
    ...
    @KasonCacheKeyPrefix(WordConstants.CACHE_KEY_PREFIX_OPERATE.METHOD_WORD_NAME)
    @Cacheable(cacheNames = WordConstants.CACHE_NAMES, keyGenerator = CacheConstants.CACHE_KEY_GENERATOR_BEAN,
            unless = "#result == null")
    public WordQueryVO queryWord(@KasonCacheKey String wordName, Integer... infoType) {
        WordQueryVO vo = new WordQueryVO();
        WordMainDO word = mainService.getOneAndCatch(wordName, infoType);
        // if you can't find the result after the tense is determined, insert a record into the queue to
        // be fetched
        if (word == null) {
        ...
        KasonAssertUtils.resourceNotNull(word, "No results for [{}]!", wordName);
        ...
        if (word.getInfoType() == WordTypeEnum.PHRASE.getType()) {
            ...
            KasonAssertUtils.resourceNotEmpty(characterVOList, "No characterVOList for [{}]!", wordName);
        ...
        try {
            vo.setCharacterVOList(
                    KasonAssertUtils.resourceNotEmpty(assembleWordQueryVO(wordName, wordId), "No characterVOList for [{}]!", wordName)
            );
        } catch (Exception e) {
    ...
    public IPage<WordQueryVO> queryWordByCh(String chineseParaphrase, int current, int size) {
        ...
        if (KasonCollectionUtils.isEmpty(page.getRecords())) {
    ...
    private List<CharacterVO> assembleWordQueryVO(String wordName, Integer wordId) throws ServiceException {
        ...
        KasonAssertUtils.assertNotEmpty(characterList, "No character for [wordName={},wordId={}]!", wordName, wordId);
        ...
        for (CharacterDO character : characterList) {
            ...
            if (KasonCollectionUtils.isEmpty(paraphraseVOList)) {
                continue;
            }
            ...
            if (KasonCollectionUtils.isEmpty(pronunciationVOList)) {
                continue;
            }
    ...
    private List<ParaphraseVO> assembleParaphraseVOList(Integer characterId) {
        ...
        for (ParaphraseDO paraphrase : paraphraseDOList) {
            ...
            if (1 == paraphrase.getIsHavePhrase()) {
                ...
                if (KasonCollectionUtils.isNotEmpty(phraseDOList)) {
            ...
            List<ParaphraseExampleVO> exampleVOList = assembleExampleVOList(vo.getParaphraseId());
    ...
    @Override
    @KasonCacheKeyPrefix(WordConstants.CACHE_KEY_PREFIX_OPERATE.METHOD_PARAPHRASE_ID)
    @Cacheable(cacheNames = WordConstants.CACHE_NAMES, keyGenerator = CacheConstants.CACHE_KEY_GENERATOR_BEAN,
            unless = "#result == null")
    public ParaphraseVO findParaphraseVO(@KasonCacheKey Integer paraphraseId) {
        ParaphraseVO vo = new ParaphraseVO();
        List<ParaphraseExampleVO> exampleVOList = new ArrayList<>();
        ...
        List<ParaphrasePhraseDO> phraseList = phraseService.list(Wrappers.<ParaphrasePhraseDO>lambdaQuery()
                .eq(ParaphrasePhraseDO::getParaphraseId, paraphrase.getParaphraseId())
                .eq(ParaphrasePhraseDO::getIsValid, GlobalConstants.FLAG_YES));
        if (KasonCollectionUtils.isNotEmpty(phraseList)) {
        ...
        if (KasonCollectionUtils.isNotEmpty(pronunciationList)) {
            List<PronunciationVO> pronunciationVOList = new ArrayList<>();
    ...
    public boolean insertVariant(String inputWordName, String fetchWordName) {
        if (KasonStringUtils.equals(inputWordName, fetchWordName)) {
            return false;
        }
    ...
    public void evict(@KasonCacheKey String wordName, WordMainDO one) {
        ...
        if (KasonCollectionUtils.isNotEmpty(list)) {
    ...
    @KasonCacheKeyPrefix(WordConstants.CACHE_KEY_PREFIX_OPERATE.METHOD_PARAPHRASE_ID)
    @CacheEvict(cacheNames = WordConstants.CACHE_NAMES, keyGenerator = CacheConstants.CACHE_KEY_GENERATOR_BEAN)
    private void evictParaphrase(@KasonCacheKey Integer paraphraseId) {
     ...
     */
    @Override
    @KasonCacheKeyPrefix(WordConstants.CACHE_KEY_PREFIX_OPERATE.METHOD_FETCH_REPLACE)
    @Cacheable(cacheNames = WordConstants.CACHE_NAMES, keyGenerator = CacheConstants.CACHE_KEY_GENERATOR_BEAN,
            unless = "#result == null")
    public FetchWordReplaceDTO getCacheReplace(@KasonCacheKey String wordName) {
    ...
    @CachePut(cacheNames = WordConstants.CACHE_NAMES, keyGenerator = CacheConstants.CACHE_KEY_GENERATOR_BEAN,
            unless = "#result == null")
    public FetchWordReplaceDTO cacheReplace(@KasonCacheKey String wordName, FetchWordReplaceDTO dto) {
        return dto;
    }

    @KasonCacheKeyPrefix(WordConstants.CACHE_KEY_PREFIX_OPERATE.METHOD_FETCH_REPLACE)
    @CacheEvict(cacheNames = WordConstants.CACHE_NAMES, keyGenerator = CacheConstants.CACHE_KEY_GENERATOR_BEAN)
    private void cacheEvictFetchReplace(@KasonCacheKey String wordName) {
    }
    ...
    public Set<WordMainDO> collectDirtyData(Integer queueId, String wordName) {
        ...
        if (KasonCollectionUtils.isEmpty(set)) {
            ...
            if (KasonCollectionUtils.isNotEmpty(wordIds)) {
