...
import me.fengorz.kason.word.api.common.enumeration.ReviseDailyCounterTypeEnum;
import me.fengorz.kason.word.api.common.enumeration.RevisePermanentAudioEnum;
import me.fengorz.kason.word.api.entity.*;
import me.fengorz.kason.word.api.model.ParaphraseTtsGenerationPayload;
import me.fengorz.kason.word.api.vo.WordReviewDailyCounterVO;
import me.fengorz.kason.word.biz.mapper.*;
import me.fengorz.kason.word.biz.service.base.ReviewAudioGenerationService;
import me.fengorz.kason.word.biz.service.base.ReviewAudioService;
import me.fengorz.kason.word.biz.service.initialing.RevisePermanentAudioHelper;
...
public class ReviewServiceImpl implements ReviewService {
    ...
    public WordReviewDailyCounterVO findReviewCounterVO(int userId, int type) {
        return KasonBeanUtils.convertFrom(findReviewCounterDO(userId, type), WordReviewDailyCounterVO.class);
    }

    @Override
    public List<WordReviewDailyCounterVO> listReviewCounterVO(int userId) {
        return KasonBeanUtils.convertFrom(
    ...
    @KasonCacheKeyPrefix(WordConstants.CACHE_KEY_PREFIX_REVIEW.METHOD_REVIEW_AUDIO)
    @Cacheable(cacheNames = WordConstants.CACHE_NAMES, keyGenerator = CacheConstants.CACHE_KEY_GENERATOR_BEAN,
            unless = "#result == null")
    public WordReviewAudioDO findWordReviewAudio(@KasonCacheKey(1) Integer sourceId, @KasonCacheKey(2) Integer type)
            throws DfsOperateException, TtsException, DataCheckedException {
    ...
    public void evictWordReviewAudio(@KasonCacheKey(1) Integer sourceId, @KasonCacheKey(2) Integer type) {
    }
    ...
    private WordReviewAudioDO generateUseTts(Integer sourceId, ReviseAudioTypeEnum type, TtsSourceEnum sourceType)
        ...
        try {
            ...
            KasonAssertUtils.assertNotEmpty(uploadResult, "uploadResult must not be empty");
    ...
    private void generateComboFromParaphraseId(Integer paraphraseId) {
        ...
        byte[] mergedBytes = KasonArrayUtils.mergeUseByteBuffer(buffer.toArray(new byte[buffer.size()][]));
        String uploadResult = null;
        try {
