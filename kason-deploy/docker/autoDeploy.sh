...
echo "KASON_DEPLOY_MODE(env): ${KASON_DEPLOY_MODE:-<unset>}"
...
if [[ "${KASON_DEPLOY_MODE}" == "-mode=obm" || "${KASON_DEPLOY_MODE}" == "-mode=osj" ]]; then
  echo "OBM/OSJ detected via KASON_DEPLOY_MODE env — skipping docker image build and deployment."
  exit 0
fi
...
# autoDeploy.sh is in: microservice-kason/kason-deploy/docker/
# project root: three levels up from kason-deploy/docker (adjust if layout changes)
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
DOCKER_DEPLOY_DIR="$PROJECT_ROOT/microservice-kason/kason-deploy/docker"
...
DOCKER_KASON_ROOT="$PROJECT_ROOT/docker/kason"
...
for d in eureka config upms auth gate word crawler ai tools; do
  if [ ! -d "$DOCKER_KASON_ROOT/$d" ]; then
     echo "❌ Missing expected Docker build directory: $DOCKER_KASON_ROOT/$d"
     ((missing++))
  fi
done
...
echo "=== KASON MICROSERVICE DEPLOYMENT WITH DOCKER COMPOSE ==="
...
cleanup_existing() {
    ...
    $COMPOSE_CMD --project-name kason-service -f "$COMPOSE_SERVICE_YML" down --remove-orphans 2>/dev/null || true
    $COMPOSE_CMD --project-name kason-base -f "$COMPOSE_BASE_YML" down --remove-orphans 2>/dev/null || true
...
build_images() {
    ...
    echo "This process will build all microservice components for the Kason platform"
    echo "Estimated time: 5-15 minutes depending on your system"
    echo

    echo "📡 Building Eureka Service Discovery Server..."
    docker build -f "$DOCKER_KASON_ROOT/eureka/Dockerfile" -t kason-eureka:2.0 "$DOCKER_KASON_ROOT/eureka/"
    echo "   ✅ Eureka server built successfully"
    echo

    echo "⚙️  Building Configuration Management Service..."
    docker build -f "$DOCKER_KASON_ROOT/config/Dockerfile" -t kason-config:2.0 "$DOCKER_KASON_ROOT/config/"
    echo "   ✅ Config service built successfully"
    echo

    echo "👥 Building User Permission Management System (UPMS)..."
    docker build -f "$DOCKER_KASON_ROOT/upms/Dockerfile" -t kason-upms:2.0 "$DOCKER_KASON_ROOT/upms/"
    echo "   ✅ UPMS service built successfully"
    echo

    echo "🔐 Building Authentication Service..."
    docker build -f "$DOCKER_KASON_ROOT/auth/Dockerfile" -t kason-auth:2.0 "$DOCKER_KASON_ROOT/auth/"
    echo "   ✅ Auth service built successfully"
    echo

    echo "🚪 Building API Gateway Service..."
    docker build -f "$DOCKER_KASON_ROOT/gate/Dockerfile" -t kason-gate:2.0 "$DOCKER_KASON_ROOT/gate/"
    echo "   ✅ Gateway service built successfully"
    echo

    echo "📝 Building Word Processing Business Service..."
    docker build -f "$DOCKER_KASON_ROOT/word/biz/Dockerfile" -t kason-word-biz:2.0 "$DOCKER_KASON_ROOT/word/"
    echo "   ✅ Word processing service built successfully"
    echo

    echo "🕷️  Building Web Crawler Service..."
    docker build -f "$DOCKER_KASON_ROOT/crawler/Dockerfile" -t kason-crawler:2.0 "$DOCKER_KASON_ROOT/crawler/"
    echo "   ✅ Crawler service built successfully"
    echo

    echo "🤖 Building AI Business Logic Service..."
    docker build -f "$DOCKER_KASON_ROOT/ai/biz/Dockerfile" -t kason-ai-biz:2.0 "$DOCKER_KASON_ROOT/ai/biz"
    echo "   ✅ AI business service built successfully"
    echo

    echo "⚡ Building AI Batch Processing Service..."
    docker build -f "$DOCKER_KASON_ROOT/ai/batch/Dockerfile" -t kason-ai-biz-batch:2.0 "$DOCKER_KASON_ROOT/ai/batch"
    echo "   ✅ AI batch service built successfully"
    echo

    echo "🧰 Building Tools Service..."
    docker build -f "$DOCKER_KASON_ROOT/tools/Dockerfile" -t kason-tools-biz:2.0 "$DOCKER_KASON_ROOT/tools/"
...
deploy_services() {
    ...
    $COMPOSE_CMD --project-name kason-base -f "$COMPOSE_BASE_YML" up -d --remove-orphans --build
    ...
    $COMPOSE_CMD --project-name kason-service -f "$COMPOSE_SERVICE_YML" up -d --force-recreate --remove-orphans --build
...
show_status() {
    ...
    $COMPOSE_CMD --project-name kason-base -f "$COMPOSE_BASE_YML" ps

    echo -e "\nApplication services status:"
    $COMPOSE_CMD --project-name kason-service -f "$COMPOSE_SERVICE_YML" ps
}
