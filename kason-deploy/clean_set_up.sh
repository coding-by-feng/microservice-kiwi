...
# Kason Microservice Cleanup Script for Raspberry Pi OS
# This script cleans up progress tracking and configuration files
# Updated: manage files under /root/microservice-kason/kason-deploy to match setup
...
SETUP_BASE_DIR="/root/microservice-kason/kason-deploy"
...
echo "=================================="
echo "Kason Microservice Setup Cleanup"
...
backup_files() {
    ...
    local backup_dir="$SETUP_BASE_DIR/kason_setup_backups"

    echo "Creating backup directory: $backup_dir"
    mkdir -p "$backup_dir"

    if [ -f "$PROGRESS_FILE" ]; then
        local progress_backup="$backup_dir/.kason_setup_progress_$timestamp"
    ...
    if [ -f "$CONFIG_FILE" ]; then
        local config_backup="$backup_dir/.kason_setup_config_$timestamp"
    ...
    if [ -f "$LOG_FILE" ]; then
        local log_backup="$backup_dir/.kason_setup.log_$timestamp"
