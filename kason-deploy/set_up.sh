...
# --- Permission bootstrap (self-healing) ---
{
  ...
  DEPLOY_ROOT="$SCRIPT_DIR"  # set_up.sh sits directly under kason-deploy
...
# Enhanced Kason Microservice Setup Script for Raspberry Pi OS
...
PROGRESS_FILE="$SETUP_BASE_DIR/.kason_setup_progress"
CONFIG_FILE="$SETUP_BASE_DIR/.kason_setup_config"
LOG_FILE="$SETUP_BASE_DIR/.kason_setup.log"
...
echo "=================================="
echo "Enhanced Kason Microservice Setup for Raspberry Pi"
...
get_env_vars() {
    ...
    # Load KASON_ENC_PASSWORD
    if has_config "KASON_ENC_PASSWORD"; then
        KASON_ENC_PASSWORD=$(load_config "KASON_ENC_PASSWORD")
        print_info "Using saved KASON_ENC_PASSWORD"
    else
        prompt_for_input "Enter KASON_ENC_PASSWORD" "KASON_ENC_PASSWORD" "true"
        save_config "KASON_ENC_PASSWORD" "$KASON_ENC_PASSWORD"
    fi
...
show_step_status() {
    ...
    if [ -n "$DOCKER_CMD" ]; then
        ...
        declare -a CONTAINERS=(
            "kason-mysql:MySQL Database:3306"
            "kason-redis:Redis Cache:6379"
            "kason-rabbit:RabbitMQ Message Queue:5672,15672"
            "tracker:FastDFS Tracker:22122"
            "storage:FastDFS Storage:23000,${FASTDFS_PORT_EXPECTED}"
            "kason-es:Elasticsearch:9200,9300"
            "kason-ui:Web UI (Nginx):80"
            "kason-ftp:FTP Server:21,21100-21110"
        )
...
execute_step_9_directories_created() {
    if ! is_step_completed "directories_created"; then
        ...
        cd "$SCRIPT_HOME"
        run_as_user mkdir -p microservice-kason docker storage_data store_path tracker_data
        run_as_user mkdir -p docker/kason docker/ui docker/rabbitmq docker/mysql
        cd "$SCRIPT_HOME/docker/kason"
...
execute_step_10_git_setup() {
    if ! is_step_completed "git_setup"; then
        ...
        cd "$SCRIPT_HOME/microservice-kason/"
        if [ -d ".git" ]; then
        ...
        if [ ! -d ".git" ]; then
            ...
            run_as_user_home git remote add origin https://github.com/coding-by-feng/microservice-kason.git
        fi
        ...
        cd "$SCRIPT_HOME"
        run_as_user ln -sf "$SCRIPT_HOME/microservice-kason/kason-deploy/docker/deployKason.sh" "$SCRIPT_HOME/easy-deploy"
        run_as_user ln -sf "$SCRIPT_HOME/microservice-kason/kason-deploy/docker/stopAll.sh" "$SCRIPT_HOME/easy-stop"
        run_as_user ln -sf "$SCRIPT_HOME/microservice-kason/kason-deploy/kason-ui/auto_deploy_ui.sh" "$SCRIPT_HOME/easy-deploy-ui"
        run_as_user ln -sf "$SCRIPT_HOME/microservice-kason/kason-deploy/kason-ui/initial_ui.sh" "$SCRIPT_HOME/easy-ui-initial"
        run_as_user ln -sf "$SCRIPT_HOME/microservice-kason/kason-deploy/docker/checkContainers.sh" "$SCRIPT_HOME/easy-check"
...
execute_step_11_hosts_configured() {
    if ! is_step_completed "hosts_configured"; then
        ...
        sed -i '/# Kason Infrastructure Services/,/# End Kason Services/d' /etc/hosts
        tee -a /etc/hosts > /dev/null << EOF

# Kason Infrastructure Services
$FASTDFS_NON_LOCAL_IP    fastdfs.fengorz.me
$INFRASTRUCTURE_IP    kason-ui
$INFRASTRUCTURE_IP    kason-redis
$INFRASTRUCTURE_IP    kason-rabbitmq
$INFRASTRUCTURE_IP    kason-db
$INFRASTRUCTURE_IP    kason-fastdfs
$INFRASTRUCTURE_IP    kason-es
$INFRASTRUCTURE_IP    kason-chattts
$INFRASTRUCTURE_IP    kason-ftp

# Kason Microservices
$SERVICE_IP    kason-microservice-local
$SERVICE_IP    kason-microservice
$SERVICE_IP    kason-eureka
$SERVICE_IP    kason-config
$SERVICE_IP    kason-auth
$SERVICE_IP    kason-upms
$SERVICE_IP    kason-gate
$SERVICE_IP    kason-ai
$SERVICE_IP    kason-crawler
# End Kason Services
EOF
...
execute_step_12_env_vars_setup() {
    if ! is_step_completed "env_vars_setup"; then
        ...
        run_as_user sed -i '/export KASON_ENC_PASSWORD=/d' "$SCRIPT_HOME/.bashrc" 2>/dev/null || true
        ...
        {
            echo ""
            echo "# Kason Microservice Environment Variables"
            echo "export KASON_ENC_PASSWORD=\"$KASON_ENC_PASSWORD\""
...
execute_step_13_ytdlp_download() {
    if ! is_step_completed "ytdlp_download"; then
        ...
        chmod +x "$tmp_file"
        run_as_user cp "$tmp_file" "$SCRIPT_HOME/docker/kason/ai/biz/yt-dlp_linux"
        run_as_user cp "$tmp_file" "$SCRIPT_HOME/docker/kason/ai/batch/yt-dlp_linux"
        rm -f "$tmp_file"
...
execute_step_14_mysql_setup() {
    if ! is_step_completed "mysql_setup"; then
        ...
        docker stop kason-mysql 2>/dev/null || true
        docker rm kason-mysql 2>/dev/null || true
        ...
        MYSQL_READY=false
        for i in {1..30}; do
            if docker exec kason-mysql mysqladmin ping -h localhost -u root -p"$MYSQL_ROOT_PASSWORD" 2>/dev/null; then
                MYSQL_READY=true
                break
            fi
            sleep 2
        ...
        if [ "$MYSQL_READY" = true ]; then
            ...
            if [ -f "$SCRIPT_HOME/docker/mysql/kason-db.sql" ]; then
                print_info "Restoring database backup..."
                docker exec -i kason-mysql mysql -u root -p"$MYSQL_ROOT_PASSWORD" kason_db < "$SCRIPT_HOME/docker/mysql/kason-db.sql"
        ...
        check_and_start_container "kason-mysql" "mysql_setup"
    fi
}
...
execute_step_15_redis_setup() {
    if ! is_step_completed "redis_setup"; then
        ...
        docker stop kason-redis 2>/dev/null || true
        docker rm kason-redis 2>/dev/null || true
        docker run -itd \
            --name kason-redis \
            -p 6379:6379 \
            --restart=unless-stopped \
        ...
        check_and_start_container "kason-redis" "redis_setup"
    fi
}
...
execute_step_16_rabbitmq_setup() {
    if ! is_step_completed "rabbitmq_setup"; then
        ...
        docker stop kason-rabbit 2>/dev/null || true
        docker rm kason-rabbit 2>/dev/null || true
        docker run -d \
            --hostname kason-rabbit \
            --name kason-rabbit \
            -p 5672:5672 \
            -p 15672:15672 \
        ...
        check_and_start_container "kason-rabbit" "rabbitmq_setup"
    fi
}
...
execute_step_18_maven_lib_install() {
    if ! is_step_completed "maven_lib_install"; then
        ...
        LIB_PATH="$SCRIPT_HOME/microservice-kason/kason-common/kason-common-tts/lib"
...
execute_step_19_deployment_setup() {
    if ! is_step_completed "deployment_setup"; then
        ...
        cd "$SCRIPT_HOME/microservice-kason/kason-deploy/docker"
        if [ ! -L "$SCRIPT_HOME/deployKason.sh" ]; then
            run_as_user cp deployKason.sh "$SCRIPT_HOME/"
            run_as_user chmod +x "$SCRIPT_HOME/deployKason.sh"
        fi
...
execute_step_20_elasticsearch_setup() {
    if ! is_step_completed "elasticsearch_setup"; then
        ...
        docker stop kason-es 2>/dev/null || true
        docker rm kason-es 2>/dev/null || true
        ...
        docker exec -u elasticsearch kason-es /usr/share/elasticsearch/bin/elasticsearch-users useradd root -p "$ES_ROOT_PASSWORD" -r superuser \
          || docker exec -u elasticsearch kason-es /usr/share/elasticsearch/bin/elasticsearch-users passwd root -p "$ES_ROOT_PASSWORD"
        docker exec -u elasticsearch kason-es /usr/share/elasticsearch/bin/elasticsearch-users useradd "$ES_USER_NAME" -p "$ES_USER_PASSWORD" -r superuser \
          || docker exec -u elasticsearch kason-es /usr/share/elasticsearch/bin/elasticsearch-users passwd "$ES_USER_NAME" -p "$ES_USER_PASSWORD"
        docker exec kason-es curl -X PUT "localhost:9200/kason_vocabulary" \
        ...
        check_and_start_container "kason-es" "elasticsearch_setup"
    fi
}
...
execute_step_22_nginx_ui_setup() {
    if ! is_step_completed "nginx_ui_setup"; then
        ...
        docker pull nginx
        docker stop kason-ui 2>/dev/null || true
        docker rm kason-ui 2>/dev/null || true
        cd "$SCRIPT_HOME"
        if [ -f "$SCRIPT_HOME/microservice-kason/kason-deploy/kason-ui/Dockerfile" ]; then
            docker build -f "$SCRIPT_HOME/microservice-kason/kason-deploy/kason-ui/Dockerfile" \
                -t kason-ui:1.0 "$SCRIPT_HOME/docker/ui/"
        else
            print_warning "UI Dockerfile not found, using nginx directly"
            docker tag nginx:latest kason-ui:1.0
        fi
        docker run -d \
            --name kason-ui \
            -p 80:80 \
        ...
        check_and_start_container "kason-ui" "nginx_ui_setup"
    fi
}
...
execute_step_23_ftp_setup() {
    if ! is_step_completed "ftp_setup"; then
        ...
        print_info "Building and starting kason-ftp container..."
        FTP_USER="$FTP_USER_CFG" FTP_PASS="$FTP_PASS_CFG" FTP_BASE_DIR="$FTP_BASE_DIR_VAL" "$FTP_INIT_SCRIPT"

        # Verify container
        sleep 2
        if docker ps --format '{{.Names}}' 2>/dev/null | grep -q '^kason-ftp$'; then
            print_success "FTP container 'kason-ftp' is running"
        else
        ...
        check_and_start_container "kason-ftp" "ftp_setup"
    fi
}
...
$CUR_IP    kason-ui
$CUR_IP    kason-redis
$CUR_IP    kason-rabbitmq
$CUR_IP    kason-db
$CUR_IP    kason-fastdfs
$CUR_IP    kason-es
$CUR_IP    kason-chattts
$CUR_IP    kason-ftp

# Kason Microservices
$CUR_SERVICE_IP    kason-microservice-local
$CUR_SERVICE_IP    kason-microservice
$CUR_SERVICE_IP    kason-eureka
$CUR_SERVICE_IP    kason-config
$CUR_SERVICE_IP    kason-auth
$CUR_SERVICE_IP    kason-upms
$CUR_SERVICE_IP    kason-gate
$CUR_SERVICE_IP    kason-ai
$CUR_SERVICE_IP    kason-crawler
# End Kason Services
EOF
...
display_final_summary() {
    ...
    echo "  $SCRIPT_HOME/easy-deploy     - Deploy Kason services"
    ...
    CONTAINERS=("kason-mysql" "kason-redis" "kason-rabbit" "tracker" "storage" "kason-es" "kason-ui" "kason-ftp")
    RUNNING_COUNT=0
    ...
    echo "  Kason UI: http://$INFRASTRUCTURE_IP:80"
...
# Final message
echo
print_success "Enhanced Kason Microservice setup completed!"
