...
import me.fengorz.kason.common.api.ApiContants;
...
import me.fengorz.kason.common.sdk.exception.dfs.DfsOperateDeleteException;
...
import me.fengorz.kason.word.api.dto.queue.result.*;
import me.fengorz.kason.word.api.entity.*;
import me.fengorz.kason.word.api.vo.ParaphraseStarListVO;
...
import me.fengorz.kason.word.biz.util.WordBizUtils;
import me.fengorz.kason.word.biz.util.WordDataSetupUtils;
...
public class CrawlerServiceImpl implements CrawlerService {
    ...
    private void subStoreFetchWordResult(FetchWordResultDTO fetchDTO, WordMainDO word) {
        ...
        if (CollUtil.isNotEmpty(codeDTOList)) {
            for (FetchWordCodeDTO codeDTO : codeDTOList) {
                ...
                KasonBeanUtils.copyProperties(codeDTO, character);
                ...
                paraphraseDTOList.forEach(paraphraseDTO -> {
                    ...
                    KasonBeanUtils.copyProperties(paraphraseDTO, paraphrase);
                    ...
                    if (KasonCollectionUtils.isNotEmpty(phraseDTOList)) {
                    ...
                    Optional.ofNullable(paraphraseDTO.getExampleDTOList())
                            .ifPresent(list -> list.forEach(exampleDTO -> {
                                ...
                                KasonBeanUtils.copyProperties(exampleDTO, example);
    ...
    public boolean handlePhrasesFetchResult(FetchPhraseRunUpResultDTO dto) {
        for (String phrase : dto.getPhrases()) {
            // 词组是word本身跳过
            if (KasonStringUtils.equals(phrase, dto.getWord())) {
                continue;
            }
            // 包含空格说明是词组
            if (KasonStringUtils.containsBlank(phrase)) {
    ...
    public boolean storePhrasesFetchResult(FetchPhraseResultDTO dto) {
        ...
        if (KasonCollectionUtils.isNotEmpty(relatedWords)) {
            for (String relatedWord : relatedWords) {
        ...
        if (KasonCollectionUtils.isEmpty(paraphrases)) {
            return false;
        }
        ...
        for (FetchParaphraseDTO paraphrase : paraphrases) {
            ...
            KasonBeanUtils.copyProperties(paraphrase, paraphraseDO);
            ...
            Optional.ofNullable(paraphrase.getExampleDTOList()).ifPresent(examples -> {
                for (FetchParaphraseExampleDTO example : examples) {
                    ...
                    KasonBeanUtils.copyProperties(example, exampleDO);
    ...
    public void generateTtsVoice(ReviseAudioGenerationEnum type) {
        try {
            ...
            if (!deepgramTtsService.hasValidApiKey()) {
    ...
    private void fetchPronunciationVoice(PronunciationDO pronunciation) {
        ...
        if (KasonStringUtils.isBlank(voiceUrl)) {
            return;
        }
        ...
        try {
            ...
            throw new ServiceException(
                    KasonStringUtils.format("fetchPronunciationVoice error, pronunciation.url={}", voiceUrl));
        }
    }
