...
public class ErrorResumeProducer extends AbstractProducer implements MQProducer {
    ...
    private void resumeDelPronunciationError() {
        log.info("resumeDelPronunciationError is beginning.");
        ...
        log.info("resumeDelPronunciationError is over.");
    }
    ...
    private void resumeOverlap() {
        ...
        synchronized (barrier) {
            Optional.ofNullable(bizAPI.listOverlapInUnLock()).ifPresent(wordNameList -> {
                ...
                wordList.forEach(
                    wordName -> Optional.ofNullable(bizAPI.getOneByWordName(wordName).getData()).ifPresent(list::add));
            });
        }
        if (KiwiCollectionUtils.isEmpty(list)) {
            return;
