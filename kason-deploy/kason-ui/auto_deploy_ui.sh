...
# UI Deployment Script for Kason Docker Container
# This script updates the UI files and restarts the kason-ui container
# Modes:
...
deploy_ui() {
    ...
    # Step 6: Restart kason-ui docker container
    echo "🔄 Restarting kason-ui docker container..."
    if docker ps --format "table {{.Names}}" | grep -q "kason-ui"; then
        docker restart kason-ui
        echo "✅ kason-ui container restarted successfully"
    else
        echo "⚠️  Warning: kason-ui container not found or not running"
        echo "   Available containers:"
        docker ps --format "table {{.Names}}\t{{.Status}}"
        echo ""
        echo "   You may need to start it manually with: docker start kason-ui"
    fi

    echo ""
    echo "🎉 UI deployment completed successfully!"
    echo "   Your new UI files are now active in the kason-ui container."
}
