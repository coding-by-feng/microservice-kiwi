...
# Ensure all kason-deploy scripts are executable and common helper binaries are +x
{
  SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  DEPLOY_ROOT="$(cd "$SCRIPT_DIR/.." && pwd 2>/dev/null || echo "$SCRIPT_DIR")"
  # Make every .sh under kason-deploy executable
...
download_latest_ytdlp() {
  ...
  local dest_biz="$CURRENT_DIR/docker/kason/ai/biz"
  local dest_batch="$CURRENT_DIR/docker/kason/ai/batch"
...
check_sudo_e() {
    ...
    if [ -z "$KASON_ENC_PASSWORD" ]; then
        missing_vars+=("KASON_ENC_PASSWORD")
        env_preserved=false
    fi
...
check_sudo_e_alternative() {
    ...
    if [ "$EUID" -eq 0 ] && [ -n "$SUDO_USER" ]; then
        ...
        if [ "$HOME" = "$expected_home" ] || [ "$HOME" = "/root" ]; then
            ...
            if [ -n "$KASON_ENC_PASSWORD" ] || [ -n "$DB_IP" ]; then
...
comprehensive_sudo_check() {
    ...
    local critical_vars=("KASON_ENC_PASSWORD" "DB_IP" "GROK_API_KEY" "MYSQL_ROOT_PASSWORD")
...
ensure_execute_permissions() {
  ...
  chmod 777 "$CURRENT_DIR/microservice-kason/kason-deploy/docker/"*.sh 2>/dev/null || true
  chmod 777 "$CURRENT_DIR/microservice-kason/kason-deploy/kason-ui/"*.sh 2>/dev/null || true
}
...
echo "=============================================="
echo "KASON_ENC_PASSWORD: $KASON_ENC_PASSWORD"
echo "DB_IP: $DB_IP"
...
cd "$CURRENT_DIR/microservice-kason/" || { echo "CRITICAL ERROR: Failed to change directory to $CURRENT_DIR/microservice-kason/"; exit 1; }
...
declare -A MICROSERVICES=(
    ...
    ["gate"]="kason-gateway"
    ["word"]="kason-word-biz"
    ["crawler"]="kason-word-crawler"
    ["ai"]="kason-ai-biz"
    ["tools"]="kason-tools-biz"   # NEW: tools
)
...
for arg in "$@"; do
  case "$arg" in
    ...
    -mode=obm)
      ...
      echo "🧱 OBM MODE: Only Maven build and copy jars to ~/built_jar (will stop all kason containers first)"
      ;;
    -mode=ouej)
      MODE="$arg"
      SKIP_GIT=true
...
send_jars_remote() {
  ...
  local cfg_dir="${original_home}/.kason"
...
EOF
  ...
  elif [ "$REMOTE_DIR" = "~/kason_jars" ]; then
    echo "ℹ️  Updating REMOTE_DIR from ~/kason_jars to ~/built_jar"
    REMOTE_DIR="~/built_jar"
  fi
...
if [ "$ONLY_GIT_MODE" = true ]; then
  ...
  cd "$CURRENT_DIR/microservice-kason/" || { echo "❌ Cannot cd to project root"; exit 1; }
  git stash || true
  git pull --rebase || git pull

  # NEW: After git-only update, ensure kason-deploy shell scripts are executable
  echo "🔐 Updating permissions for kason-deploy shell scripts..."
  DEPLOY_DIR="$CURRENT_DIR/microservice-kason/kason-deploy"
  if [ -d "$DEPLOY_DIR" ]; then
    # Recursively set 777 for all .sh files under kason-deploy (portable for macOS/BSD find)
    find "$DEPLOY_DIR" -type f -name "*.sh" -exec chmod 777 {} \;
    echo "✅ Set 777 on all .sh files under: $DEPLOY_DIR"
  else
    echo "ℹ️  kason-deploy directory not found at: $DEPLOY_DIR"
  fi
...
if [ "$ONLY_BUILD_MAVEN" = true ]; then
  ...
  echo "🛑 Stopping all kason containers before OBM build..."
  "$CURRENT_DIR/microservice-kason/kason-deploy/docker/stopAll.sh" || true
  ...
  cd "$CURRENT_DIR/microservice-kason/kason-common/kason-common-tts/lib"
  ...
  cd "$CURRENT_DIR/microservice-kason/"
  ...
  if [ "$BUILD_ALL_SERVICES" = true ]; then
    ...
    mvn -q clean install -pl kason-common -am -Dmaven.test.skip=true -B -Dmaven.repo.local="$ORIGINAL_HOME/.m2/repository"
    IFS=',' read -ra SERVICE_ARRAY <<< "$SELECTED_SERVICES"
    for service in "${SERVICE_ARRAY[@]}"; do
      case "$service" in
        "upms")
          mvn -q clean install -pl kason-upms/kason-upms-biz -am -Dmaven.test.skip=true -B -Dmaven.repo.local="$ORIGINAL_HOME/.m2/repository"
          ;;
        "word")
          mvn -q clean install -pl kason-word/kason-word-biz -am -Dmaven.test.skip=true -B -Dmaven.repo.local="$ORIGINAL_HOME/.m2/repository"
          ;;
        "crawler")
          mvn -q clean install -pl kason-word/kason-word-crawler -am -Dmaven.test.skip=true -B -Dmaven.repo.local="$ORIGINAL_HOME/.m2/repository"
          ;;
        "ai")
          mvn -q clean install -pl kason-ai/kason-ai-biz -am -Dmaven.test.skip=true -B -Dmaven.repo.local="$ORIGINAL_HOME/.m2/repository"
          ;;
        *)
  ...
  if should_build_service "eureka"; then
    copy_or_warn "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-eureka/2.0/kason-eureka-2.0.jar" eureka
  fi
  if should_build_service "config"; then
    copy_or_warn "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-config/2.0/kason-config-2.0.jar" config
  fi
  if should_build_service "upms"; then
    copy_or_warn "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-upms-biz/2.0/kason-upms-biz-2.0.jar" upms
  fi
  if should_build_service "auth"; then
    copy_or_warn "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-auth/2.0/kason-auth-2.0.jar" auth
  fi
  if should_build_service "gate"; then
    copy_or_warn "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-gateway/2.0/kason-gateway-2.0.jar" gateway
  fi
  if should_build_service "tools"; then
    copy_or_warn "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-tools-biz/2.0/kason-tools-biz-2.0.jar" tools
  fi
  if should_build_service "word"; then
    copy_or_warn "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-word-biz/2.0/kason-word-biz-2.0.jar" word-biz
  fi
  if should_build_service "crawler"; then
    copy_or_warn "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-word-crawler/2.0/kason-word-crawler-2.0.jar" crawler
  fi
  if should_build_service "ai"; then
    copy_or_warn "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-ai-biz/2.0/kason-ai-biz-2.0.jar" ai
  fi
...
"$CURRENT_DIR/microservice-kason/kason-deploy/docker/stopAll.sh" || {
...
if [ "$FAST_DEPLOY_MODE" = true ]; then
  ...
  if should_build_service "eureka"; then
    rm -rf "$CURRENT_DIR/docker/kason/eureka/logs/"* 2>/dev/null || true
  fi
  if should_build_service "config"; then
    rm -rf "$CURRENT_DIR/docker/kason/config/logs/"* 2>/dev/null || true
  fi
  if should_build_service "upms"; then
    rm -rf "$CURRENT_DIR/docker/kason/upms/logs/"* 2>/dev/null || true
  fi
  if should_build_service "auth"; then
    rm -rf "$CURRENT_DIR/docker/kason/auth/logs/"* 2>/dev/null || true
  fi
  if should_build_service "gate"; then
    rm -rf "$CURRENT_DIR/docker/kason/gate/logs/"* 2>/dev/null || true
  fi
  if should_build_service "word"; then
    rm -rf "$CURRENT_DIR/docker/kason/word/logs/"* 2>/dev/null || true
    rm -rf "$CURRENT_DIR/docker/kason/word/crawlerTmp/"* 2>/dev/null || true
    rm -rf "$CURRENT_DIR/docker/kason/word/bizTmp/"* 2>/dev/null || true
  fi
  if should_build_service "crawler"; then
    rm -rf "$CURRENT_DIR/docker/kason/crawler/logs/"* 2>/dev/null || true
  fi
  if should_build_service "ai"; then
    rm -rf "$CURRENT_DIR/docker/kason/ai/logs/"* 2>/dev/null || true
    rm -rf "$CURRENT_DIR/docker/kason/ai/tmp/"* 2>/dev/null || true
  fi
  if should_build_service "tools"; then
    rm -rf "$CURRENT_DIR/docker/kason/tools/logs/"* 2>/dev/null || true   # NEW
  fi
  ...
  if [ "$SKIP_DOCKER_BUILD" = false ]; then
    ...
    resolve_jar_path() {
      ...
      if [ "$USE_EXISTING_JARS_ONLY" = true ]; then
        case "$service" in
          eureka)  find_built_by_prefix "kason-eureka" "$ORIGINAL_HOME/built_jar" ;;
          config)  find_built_by_prefix "kason-config" "$ORIGINAL_HOME/built_jar" ;;
          upms)    find_built_by_prefix "kason-upms-biz" "$ORIGINAL_HOME/built_jar" ;;
          auth)    find_built_by_prefix "kason-auth" "$ORIGINAL_HOME/built_jar" ;;
          gate)    find_built_by_prefix "kason-gateway" "$ORIGINAL_HOME/built_jar" ;;
          word)    find_built_by_prefix "kason-word-biz" "$ORIGINAL_HOME/built_jar" ;;
          crawler) find_built_by_prefix "kason-word-crawler" "$ORIGINAL_HOME/built_jar" ;;
          ai)      find_built_by_prefix "kason-ai-biz" "$ORIGINAL_HOME/built_jar" ;;
          tools)   find_built_by_prefix "kason-tools-biz" "$ORIGINAL_HOME/built_jar" ;;  # NEW
          *) echo "" ;;
        esac
      else
        echo "$m2_path"
    ...
    if should_build_service "eureka"; then
      ...
      cp -f "$CURRENT_DIR/microservice-kason/kason-eureka/Dockerfile" "$CURRENT_DIR/docker/kason/eureka/"
      eureka_jar=$(resolve_jar_path "eureka" "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-eureka/2.0/kason-eureka-2.0.jar")
      ...
      cp -f "$eureka_jar" "$CURRENT_DIR/docker/kason/eureka/"
    fi

    # config
    if should_build_service "config"; then
      echo "📄 Copying config files..."
      cp -f "$CURRENT_DIR/microservice-kason/kason-config/Dockerfile" "$CURRENT_DIR/docker/kason/config/"
      config_jar=$(resolve_jar_path "config" "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-config/2.0/kason-config-2.0.jar")
      ...
      cp -f "$config_jar" "$CURRENT_DIR/docker/kason/config/"
    fi

    # upms
    if should_build_service "upms"; then
      echo "📄 Copying upms files..."
      cp -f "$CURRENT_DIR/microservice-kason/kason-upms/kason-upms-biz/Dockerfile" "$CURRENT_DIR/docker/kason/upms/"
      upms_jar=$(resolve_jar_path "upms" "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-upms-biz/2.0/kason-upms-biz-2.0.jar")
      ...
      cp -f "$upms_jar" "$CURRENT_DIR/docker/kason/upms/"
    fi

    # auth
    if should_build_service "auth"; then
      echo "📄 Copying auth files..."
      cp -f "$CURRENT_DIR/microservice-kason/kason-auth/Dockerfile" "$CURRENT_DIR/docker/kason/auth/"
      auth_jar=$(resolve_jar_path "auth" "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-auth/2.0/kason-auth-2.0.jar")
      ...
      cp -f "$auth_jar" "$CURRENT_DIR/docker/kason/auth/"
    fi

    # gate
    if should_build_service "gate"; then
      echo "📄 Copying gateway files..."
      cp -f "$CURRENT_DIR/microservice-kason/kason-gateway/Dockerfile" "$CURRENT_DIR/docker/kason/gate/"
      gate_jar=$(resolve_jar_path "gate" "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-gateway/2.0/kason-gateway-2.0.jar")
      ...
      cp -f "$gate_jar" "$CURRENT_DIR/docker/kason/gate/"
    fi
    ...
    if should_build_service "tools"; then
      ...
      if [ -f "$CURRENT_DIR/microservice-kason/kason-tools/kason-tools-biz/Dockerfile" ]; then
        cp -f "$CURRENT_DIR/microservice-kason/kason-tools/kason-tools-biz/Dockerfile" "$CURRENT_DIR/docker/kason/tools/"
      elif [ -f "$CURRENT_DIR/microservice-kason/kason-tools/kason-tools-biz/docker/Dockerfile" ]; then
        cp -f "$CURRENT_DIR/microservice-kason/kason-tools/kason-tools-biz/docker/Dockerfile" "$CURRENT_DIR/docker/kason/tools/"
      else
        echo "⚠️  Tools Dockerfile not found under kason-tools/kason-tools-biz. Ensure it exists."
      fi
      tools_jar=$(resolve_jar_path "tools" "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-tools-biz/2.0/kason-tools-biz-2.0.jar")
      ...
      cp -f "$tools_jar" "$CURRENT_DIR/docker/kason/tools/"
    fi

    # word
    if should_build_service "word"; then
      echo "📄 Copying word service files..."
      cp -f "$CURRENT_DIR/microservice-kason/kason-word/kason-word-biz/docker/biz/Dockerfile" "$CURRENT_DIR/docker/kason/word/biz"
      cp -f "$CURRENT_DIR/microservice-kason/kason-word/kason-word-biz/docker/crawler/Dockerfile" "$CURRENT_DIR/docker/kason/word/crawler"
      word_jar=$(resolve_jar_path "word" "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-word-biz/2.0/kason-word-biz-2.0.jar")
      ...
      cp -f "$word_jar" "$CURRENT_DIR/docker/kason/word/"
      cp -f "$CURRENT_DIR/gcp-credentials.json" "$CURRENT_DIR/docker/kason/word/bizTmp"
    fi

    # crawler
    if should_build_service "crawler"; then
      echo "📄 Copying crawler files..."
      cp -f "$CURRENT_DIR/microservice-kason/kason-word/kason-word-crawler/Dockerfile" "$CURRENT_DIR/docker/kason/crawler/"
      crawler_jar=$(resolve_jar_path "crawler" "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-word-crawler/2.0/kason-word-crawler-2.0.jar")
      ...
      cp -f "$crawler_jar" "$CURRENT_DIR/docker/kason/crawler/"
    fi

    # ai
    if should_build_service "ai"; then
      echo "📄 Copying AI service files..."
      cp -f "$CURRENT_DIR/microservice-kason/kason-ai/kason-ai-biz/docker/biz/Dockerfile" "$CURRENT_DIR/docker/kason/ai/biz"
      cp -f "$CURRENT_DIR/microservice-kason/kason-ai/kason-ai-biz/docker/batch/Dockerfile" "$CURRENT_DIR/docker/kason/ai/batch"
      ai_jar=$(resolve_jar_path "ai" "$ORIGINAL_HOME/.m2/repository/me/fengorz/kason-ai-biz/2.0/kason-ai-biz-2.0.jar")
      [ -n "$ai_jar" ] && [ -f "$ai_jar" ] || { echo "❌ Missing jar for ai in ${USE_EXISTING_JARS_ONLY:+$ORIGINAL_HOME/built_jar}. Use -mode=obm first."; exit 1; }
      cp -f "$ai_jar" "$CURRENT_DIR/docker/kason/ai/biz"
      cp -f "$ai_jar" "$CURRENT_DIR/docker/kason/ai/batch"
    fi
...
deploy_single_service() {
    local service=$1
    local DOCKER_DIR="$CURRENT_DIR/docker/kason"

    # Ensure Docker network exists
    docker network create kason-network 2>/dev/null || true
    ...
    case "$service" in
        "eureka")
            ...
            docker build -t kason-eureka:latest .
            docker run -d --name kason-eureka \
                --network kason-network \
                -p 18000:18000 \
                -v "$DOCKER_DIR/eureka/logs:/logs" \
                -e EUREKA_SERVER=http://kason-eureka:18000/eureka/ \
                kason-eureka:latest
        ...
        "config")
            ...
            docker build -t kason-config:latest .
            docker run -d --name kason-config \
                --network kason-network \
                -p 18001:18001 \
        ...
        "upms")
            ...
            cd "$DOCKER_DIR/upms"
            docker build -t kason-upms:latest .
            docker run -d --name kason-upms-biz \
                --network kason-network \
                -p 18002:18002 \
        ...
        "auth")
            ...
            cd "$DOCKER_DIR/auth"
            docker build -t kason-auth:latest .
            docker run -d --name kason-auth \
                --network kason-network \
                -p 18003:18003 \
        ...
        "gate")
            ...
            cd "$DOCKER_DIR/gate"
            docker build -t kason-gateway:latest .
            docker run -d --name kason-gateway \
                --network kason-network \
                -p 18004:18004 \
                -v "$DOCKER_DIR/gate/logs:/logs" \
                -e EUREKA_SERVER=http://kason-eureka:18000/eureka/ \
                kason-gateway:latest
        ...
        "word")
            ...
            docker build -t kason-word-biz:latest .
            docker run -d --name kason-word-biz \
                --network kason-network \
                -p 18010:18010 \
                -v "$DOCKER_DIR/word/logs:/logs" \
                -v "$DOCKER_DIR/word/bizTmp:/bizTmp" \
                -e EUREKA_SERVER=http://kason-eureka:18000/eureka/ \
                kason-word-biz:latest
        ...
        "crawler")
            ...
            docker build -t kason-word-crawler:latest .
            docker run -d --name kason-word-crawler \
                --network kason-network \
                -p 18011:18011 \
                -v "$DOCKER_DIR/crawler/logs:/logs" \
                -e EUREKA_SERVER=http://kason-eureka:18000/eureka/ \
                kason-word-crawler:latest
        ...
        "ai")
            ...
            if [ -d "$DOCKER_DIR/ai/biz" ] && [ -f "$DOCKER_DIR/ai/biz/Dockerfile" ]; then
                ...
                docker build -t kason-ai-biz:latest .

                echo "   → Starting AI Business Logic container..."
                docker run -d --name kason-ai-biz \
                    --network kason-network \
                    -p 18015:18015 \
            ...
            if [ -d "$DOCKER_DIR/ai/batch" ] && [ -f "$DOCKER_DIR/ai/batch/Dockerfile" ]; then
                ...
                docker build -t kason-ai-batch:latest .

                echo "   → Starting AI Batch container..."
                docker run -d --name kason-ai-batch \
                    --network kason-network \
                    -p 18016:18016 \
                    -v "$DOCKER_DIR/ai/logs:/logs" \
                    -v "$DOCKER_DIR/ai/tmp:/ai-tmp" \
        ...
        "tools")  # NEW
            ...
            cd "$DOCKER_DIR/tools"
            docker build -t kason-tools-biz:latest .
            docker run -d --name kason-tools-biz \
                --network kason-network \
                -p 18012:18012 \
                -v "$DOCKER_DIR/tools/logs:/logs" \
                -e EUREKA_SERVER=http://kason-eureka:18000/eureka/ \
                kason-tools-biz:latest
...
selective_deployment() {
    ...
    local DOCKER_KASON_ROOT="$CURRENT_DIR/docker/kason"
    IFS=',' read -ra SERVICE_ARRAY <<< "$services"
    for service in "${SERVICE_ARRAY[@]}"; do
        case "$service" in
          eureka)
            [ -d "$DOCKER_KASON_ROOT/eureka" ] && docker build -f "$DOCKER_KASON_ROOT/eureka/Dockerfile" -t kason-eureka:2.0 "$DOCKER_KASON_ROOT/eureka" || true ;;
          config)
            [ -d "$DOCKER_KASON_ROOT/config" ] && docker build -f "$DOCKER_KASON_ROOT/config/Dockerfile" -t kason-config:2.0 "$DOCKER_KASON_ROOT/config" || true ;;
          upms)
            [ -d "$DOCKER_KASON_ROOT/upms" ] && docker build -f "$DOCKER_KASON_ROOT/upms/Dockerfile" -t kason-upms:2.0 "$DOCKER_KASON_ROOT/upms" || true ;;
          auth)
            [ -d "$DOCKER_KASON_ROOT/auth" ] && docker build -f "$DOCKER_KASON_ROOT/auth/Dockerfile" -t kason-auth:2.0 "$DOCKER_KASON_ROOT/auth" || true ;;
          gate)
            [ -d "$DOCKER_KASON_ROOT/gate" ] && docker build -f "$DOCKER_KASON_ROOT/gate/Dockerfile" -t kason-gate:2.0 "$DOCKER_KASON_ROOT/gate" || true ;;
          word)
            [ -d "$DOCKER_KASON_ROOT/word/biz" ] && docker build -f "$DOCKER_KASON_ROOT/word/biz/Dockerfile" -t kason-word-biz:2.0 "$DOCKER_KASON_ROOT/word" || true ;;
          crawler)
            [ -d "$DOCKER_KASON_ROOT/crawler" ] && docker build -f "$DOCKER_KASON_ROOT/crawler/Dockerfile" -t kason-crawler:2.0 "$DOCKER_KASON_ROOT/crawler" || true ;;
          ai)
            [ -d "$DOCKER_KASON_ROOT/ai/biz" ] && docker build -f "$DOCKER_KASON_ROOT/ai/biz/Dockerfile" -t kason-ai-biz:2.0 "$DOCKER_KASON_ROOT/ai/biz" || true
            [ -d "$DOCKER_KASON_ROOT/ai/batch" ] && docker build -f "$DOCKER_KASON_ROOT/ai/batch/Dockerfile" -t kason-ai-biz-batch:2.0 "$DOCKER_KASON_ROOT/ai/batch" || true ;;
          tools)
            [ -d "$DOCKER_KASON_ROOT/tools" ] && docker build -f "$DOCKER_KASON_ROOT/tools/Dockerfile" -t kason-tools-biz:2.0 "$DOCKER_KASON_ROOT/tools" || true ;;
          *)
    ...
    if [ -n "$COMPOSE_CMD" ]; then
        ...
        declare -A COMPOSE_SERVICES=(
            ...
            [gate]="kason-gate"
            [word]="kason-word-biz"
            [crawler]="kason-crawler"
            [ai]="kason-ai-biz"
            [tools]="kason-tools-biz"
        )
        ...
        # Compose files
        local BASE_YML="$CURRENT_DIR/microservice-kason/kason-deploy/docker/docker-compose-base.yml"
        local SERV_YML="$CURRENT_DIR/microservice-kason/kason-deploy/docker/docker-compose-service.yml"
        ...
        if [ ${#BASE_TARGETS[@]} -gt 0 ]; then
            ...
            echo "🧩 docker compose (project=kason-base): ${BASE_TARGETS[*]}"
            $COMPOSE_CMD --project-name kason-base -f "$BASE_YML" up -d --no-deps --force-recreate --remove-orphans "${BASE_TARGETS[@]}"
        fi
        ...
        if [ ${#APP_TARGETS[@]} -gt 0 ]; then
            ...
            echo "🧩 docker compose (project=kason-service): ${APP_TARGETS[*]}"
            $COMPOSE_CMD --project-name kason-service -f "$SERV_YML" up -d --no-deps --force-recreate --remove-orphans "${APP_TARGETS[@]}"
        fi
    else
    ...
    echo ""
    docker ps --filter "name=kason-" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
}
...
if [ "$BUILD_ALL_SERVICES" = true ]; then
  ...
  AUTO_DEPLOY_SCRIPT="$CURRENT_DIR/microservice-kason/kason-deploy/docker/autoDeploy.sh"
  if [ ! -f "$AUTO_DEPLOY_SCRIPT" ]; then
     echo "❌ autoDeploy script not found at: $AUTO_DEPLOY_SCRIPT"
     echo "Current directory: $(pwd)"
     ls -l "$CURRENT_DIR/microservice-kason/kason-deploy/docker/" || true
     exit 1
  fi
  ...
  KASON_DEPLOY_MODE="$MODE" ONLY_BUILD_MAVEN="$ONLY_BUILD_MAVEN" bash -x "$AUTO_DEPLOY_SCRIPT" "$MODE" 2>&1 | tee "$CURRENT_DIR/autoDeploy.log"
...
if [ "$ENABLE_AUTO_CHECK" = true ]; then
  if ! pgrep -f "autoCheckService.sh" >/dev/null; then
    ...
    nohup "$CURRENT_DIR/microservice-kason/kason-deploy/docker/autoCheckService.sh" >"$CURRENT_DIR/autoCheck.log" 2>&1 &
